...
from shared.to.vcf import VcfReader, VcfWriter, Position
...
def nonsomatic_tag(args):
    ...
    disable_print_nonmosaic_calls = args.disable_print_nonmosaic_calls
    ...
    if len(pon_vcf_fn_list) > 0:
        for index, pon_vcf_fn in enumerate(pon_vcf_fn_list):
            ...
            pon_vcf_header_info = '##INFO=<ID=PoN_{},Number=0,Type=Flag,Description="file={},md5={},allele_matching={},non-mosaic variant tagged by panel of normals">'.format(index + 1, str(pon_vcf_fn), pon_vcf_md5, pon_require_allele_matching_list[index])
    ...
    with open(pileup_output_vcf_fn, 'w') as output:
        ...
        for contig in contigs_order_list:
            ...
            for pos_info in all_pos_info:
                ...
                if not disable_print_nonmosaic_calls:
...
def main():
    parser = ArgumentParser(description="Non-mosaic tagging for pileup data")
