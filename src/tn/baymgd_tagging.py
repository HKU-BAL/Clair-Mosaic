...
import os
import gzip
from shared.tn.utils import str_none, subprocess_popen
...
def parse_input_vcf_line(line, ctg_name):
    ...
    p = float(fmt_dict.get('GQ', 0))
    Phred_Trans = (-10 * log(e, 10))
    phred_p = max(Phred_Trans * log(((1.0 - p) + 1e-10) / (p + 1e-10)) + 2.0, 0.0)
    phred_gq = int(round(phred_p, 4))
    ...
    with open(output_path, 'w') as f_out:
        ...
        for var in input_vars:
            ...
            if filter == 'PASS':
                ...
                continue
            elif not args.show_germline and filter == 'Germline':
                continue
            else:
                # Write output line
                fields = var['raw_line'].strip().split('\t')
